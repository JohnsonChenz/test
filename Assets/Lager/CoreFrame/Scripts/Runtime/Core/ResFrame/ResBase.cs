...
namespace CoreFrame
{
    namespace ResFrame
    {
        public class ResBase : FrameBase
        {
            [HideInInspector] public string resName = "";       // 用於存放ResPath整個路徑名稱
            [HideInInspector] public ResType resType = null;    // 定義Res類型, 用於取決於要新增至ResRoot中哪個對應的節點
            ...
            {
                ResManager.GetInstance().Close(this.resName);
            }
        }
    }
}